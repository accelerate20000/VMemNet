...
def choose_fold(which_fold):
    ...
    if which_fold==0:
        checkpoint = './checkpoint/checkpoint_fold_0_0.5237312056529898.pth.tar'
    
    if which_fold==1:
        checkpoint = './checkpoint/checkpoint_fold_1_0.5175212760594858.pth.tar'

    if which_fold==2:
        checkpoint = './checkpoint/checkpoint_fold_2_0.5148327401644722.pth.tar'

    if which_fold==3:
        checkpoint = './checkpoint/checkpoint_fold_3_0.5206630396687968.pth.tar'

    if which_fold==4:
        checkpoint = './checkpoint/checkpoint_fold_4_0.5172841501215397.pth.tar'

    if which_fold==5:
        checkpoint = './checkpoint/checkpoint_fold_5_0.5404364794276509.pth.tar'

    if which_fold==6:
        checkpoint = './checkpoint/checkpoint_fold_6_0.5351826770676688.pth.tar'

    if which_fold==7:
        checkpoint = './checkpoint/checkpoint_fold_7_0.5610508470133193.pth.tar'
    ...
    best_val = checkpoint['val_level']
    encoder = checkpoint['encoder']
    semantic_att = checkpoint['decoder']
    spatial_att = checkpoint['AMemNet']
    fusion = checkpoint['fusion']
    encoder_optimizer = checkpoint['encoder_optimizer']
    semantic_att_optimizer = checkpoint['decoder_optimizer']
    spatial_att_optimizer = checkpoint['AMemNet_optimizer']
...
def validate(val_loader, encoder, semantic_att, spatial_att, fusion, criterion):
    ...
    with torch.no_grad():
        for step, (frames, feature_vector, score, ann, video_name) in enumerate(val_loader):
            ...
            amnet_out, beta_out = spatial_att(encoded_frames_1024)
            fusion_out = fusion(encoded_frames_1024, feature_vector)  

            preds_mix= semantic_att(fusion_out).squeeze(1)
            amnet_out=amnet_out.squeeze(1)
            pred_late_fusion = preds_mix*(0.6) + amnet_out*(0.4)
